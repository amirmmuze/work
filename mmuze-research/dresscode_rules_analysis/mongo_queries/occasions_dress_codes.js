db.getCollection('retailer_products').aggregate([
    ...
    {"$group":{"_id":{'dress_code':"$dress_code","gender":"$gender","occasion":{ "$arrayElemAt": [ "$occasion_for_dress_code.occasion", 0 ]}},"sample":{"$max":"$_id"},"count":{"$sum":1} }},
    {"$project":{"dress_code":"$_id.dress_code","gender":"$_id.gender","count":"$count" ,"_id":0,"sample":"$sample","occasion":"$_id.occasion"}},
    {"$group" : { "_id":{"gender":"$gender","occasion":"$occasion"},"keys" : { "$push" : "$dress_code" },"samples" : { "$push" : "$sample" },"sumValues" : { "$push" : "$count" },"total" : { "$sum" : "$count" }  }},
    { "$project" : {
            "dress_codes" : "$keys",
            "counts" : "$sumValues",
            "total" : "$total",
    ...
    {"$project":{"gender":"$_id.gender","occasion":"$_id.occasion","dress_codes":"$dress_codes","percentages":"$percentages","counts":"$counts" ,"total_count":"$total","_id":0,   "samples":"$samples",
        }},
