db.getCollection('retailer_products').aggregate([
    ...
    {"$group":{"_id":{'dress_code':"$dress_code","gender":"$gender","parent_type":"$parent_type","product_type":"$product_type"},"count":{"$sum":1} }},
    {"$project":{"dress_code":"$_id.dress_code","gender":"$_id.gender","parent_type":"$_id.parent_type","product_type":"$_id.product_type","count":"$count" ,"_id":0}},
    {"$group" : { "_id":{"gender":"$gender","parent_type":"$parent_type","product_type":"$product_type"},"keys" : { "$push" : "$dress_code" },"sumValues" : { "$push" : "$count" },"total" : { "$sum" : "$count" }  }},
    { "$project" : {
            "dress_codes" : "$keys",
            "counts" : "$sumValues",
            "total" : "$total",
    ...
    {"$project":{"gender":"$_id.gender","parent_type":"$_id.parent_type","product_type":"$_id.product_type","product_type":"$_id.product_type","dress_codes":"$dress_codes","percentages":"$percentages","counts":"$counts" ,"total_count":"$total","_id":0}},

    {"$out" : "product_types_dress_codes_counts"}
]);
